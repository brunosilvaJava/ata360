...
def validate_transcript(transcript_path: str | Path, repo_root: str | Path) -> dict:
    ...
    client_id = parts[1]
    project_id = parts[3]
    client_file = Path(repo_root) / "clientes" / client_id / "cliente.yml"
    project_file = Path(repo_root) / "clientes" / client_id / "projetos" / project_id / "projeto.yml"
